    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        for (int j = 0; j < 8; j++)
        {
            ...
            if (chess->board[i][j] != NULL)
            {
                ...
                if (isupper(piece))
                    strcpy(buffer, "\033[97m");
                else
                    strcpy(buffer, "\033[30m");
                uarte_write(buffer, strlen(buffer));
...
int pawn_moves(chess_board *chess, piece *p, int dest_y, int dest_x)
{
    ...
    { // The piece on the destination square is of the opposite color
        return 1;
    }
...
int rook_moves(chess_board *chess, piece *p, int dest_y, int dest_x)
{
    ...
    if (dx != 0 && dy != 0)
        return 0; // The rook can only move in straight lines
    if (dx == 0 && dy == 0)
        return 0; // The rook cannot move to the same square
    ...
    {
        if (chess->board[current_step_y][current_step_x] != NULL)
            return 0; // Another piece is blocking the rooks path
...
int bishop_moves(chess_board *chess, piece *p, int dest_y, int dest_x)
{
    ...
    if (abs(dx) != abs(dy))
        return 0; // The bishop can only move diagonally
    if (dx == 0 && dy == 0)
        return 0; // The bishop cannot move to the same square
    ...
    {
        if (chess->board[current_step_y][current_step_x] != NULL)
            return 0; // Another piece is blocking the bishops path
        current_step_x += step_dir_x;
        current_step_y += step_dir_y;
    }
...
int queen_moves(chess_board *chess, piece *p, int dest_y, int dest_x)
{
    ...
    {
        if (chess->board[current_step_y][current_step_x] != NULL)
            return 0; // Another piece is blocking the queens path
        current_step_x += step_dir_x;
        current_step_y += step_dir_y;
    }
...
int king_moves(chess_board *chess, piece *p, int dest_y, int dest_x)
{
    ...
    if (dx == 0 && dy == 0)
        return 0; // The king cannot move to the same square
    if (abs(dx) > 1 || abs(dy) > 1)
        return 0; // The king can only move one square in any direction
    if (chess->board[dest_y][dest_x] == NULL || is_white(p->type) != is_white(chess->board[dest_y][dest_x]->type))
        return 1; // The piece on the destination square is of the opposite color or is empty
...
{
    if (p == NULL)
        return 0; // Check if the piece is valid
    if (dest_y < 0 || dest_y > 7 || dest_x < 0 || dest_x > 7)
        return 0; // Checking if the destination is within the board limits

    switch (tolower(p->type))
    {
    case 'p':
        return pawn_moves(chess, p, dest_y, dest_x);
    case 'r':
        return rook_moves(chess, p, dest_y, dest_x);
    case 'n':
        return knight_moves(chess, p, dest_y, dest_x);
    case 'b':
        return bishop_moves(chess, p, dest_y, dest_x);
    case 'q':
        return queen_moves(chess, p, dest_y, dest_x);
    case 'k':
        return king_moves(chess, p, dest_y, dest_x);
    default:
        return 0;
    }
}
